    ...
    <div>
        ...
        <Card @click="click" class="cursor-pointer hover:bg-primary-200/10 flex flex-col gap-2">
            <div class="grid grid-cols-2 gap-2">
                <div v-for="image in images" :key="image.path" class="w-full rounded-sm mb-4 relative"
                    :class="{
...
import {getSettings, NSFWDisplay} from "@/stores/settings";
...
const images = computed<Attachment[]>(() => {
    let imgs: Attachment[] = props.post.attachments?.filter(a => a.is_image) || []

    if (settings.value.nsfw_display === NSFWDisplay.HIDE) {
        imgs = imgs.filter(i => !i.is_nsfw)
    } else if (settings.value.nsfw_display === NSFWDisplay.ALWAYS) {
        imgs = imgs.map(i => ({...i, is_nsfw: false}))
    }
